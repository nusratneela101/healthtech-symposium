...
try {
    $stats = [
        ...
        'unread_responses' => $unreadCount,
        'delivered'        => Database::fetchOne("SELECT COUNT(*) AS c FROM email_logs WHERE (status IN ('delivered','opened','clicked')) OR (status = 'sent' AND message_id IS NOT NULL AND message_id != '' AND sent_at < DATE_SUB(NOW(), INTERVAL 5 MINUTE))")['c'] ?? 0,
