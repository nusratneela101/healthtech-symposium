...
function generateEnvFile(array $data): array
{
    ...
    $lines = [
        'APP_NAME="' . ($data['app_name'] ?? 'Canada Fintech Symposium') . '"',
    ...
    if ($emailProvider === 'brevo') {
        $lines[] = '';
        $lines[] = '# Brevo API';
        $lines[] = 'BREVO_API_KEY=' . ($data['brevo_api_key'] ?? '');
    } elseif ($emailProvider === 'ms365') {
        $lines[] = '';
        $lines[] = '# Microsoft 365 OAuth2';
        $lines[] = 'MS_OAUTH_CLIENT_ID='     . ($data['ms_client_id'] ?? '');
        $lines[] = 'MS_OAUTH_CLIENT_SECRET=' . ($data['ms_client_secret'] ?? '');
        $lines[] = 'MS_OAUTH_TENANT_ID='     . ($data['ms_tenant_id'] ?? 'common');
        $lines[] = 'MS_OAUTH_REDIRECT_URI='  . rtrim($data['app_url'] ?? '', '/') . '/api/msgraph/callback.php';
        $lines[] = 'SMTP_HOST=smtp-mail.outlook.com';
        $lines[] = 'SMTP_PORT=587';
        $lines[] = 'SMTP_SECURE=tls';
        $lines[] = 'SMTP_USER=' . ($data['ms_smtp_user'] ?? '');
        $lines[] = 'SMTP_PASS=' . ($data['ms_smtp_pass'] ?? '');
        $lines[] = 'SMTP_FROM_EMAIL=' . ($data['ms_smtp_user'] ?? '');
        $lines[] = 'SMTP_FROM_NAME="' . ($data['app_name'] ?? 'Canada Fintech Symposium') . '"';
    } elseif ($emailProvider === 'smtp') {
        $lines[] = '';
        $lines[] = '# SMTP';
        $lines[] = 'SMTP_HOST='       . ($data['smtp_host'] ?? '');
        $lines[] = 'SMTP_PORT='       . ($data['smtp_port'] ?? '587');
        $lines[] = 'SMTP_SECURE='     . ($data['smtp_secure'] ?? 'tls');
        $lines[] = 'SMTP_USER='       . ($data['smtp_user'] ?? '');
        $lines[] = 'SMTP_PASS='       . ($data['smtp_pass'] ?? '');
        $lines[] = 'SMTP_FROM_EMAIL=' . ($data['smtp_from_email'] ?? $data['smtp_user'] ?? '');
        $lines[] = 'SMTP_FROM_NAME="' . ($data['app_name'] ?? 'Canada Fintech Symposium') . '"';
    }
    ...
    if (!empty($data['n8n_url'])) {
        ...
        $lines[] = 'N8N_URL='         . $data['n8n_url'];
        $lines[] = 'N8N_API_KEY='     . ($data['n8n_api_key'] ?? '');
        $lines[] = 'N8N_WEBHOOK_URL=' . ($data['n8n_webhook_url'] ?? '');
    }
    ...
    if (!empty($data['apollo_api_key'])) {
        $lines[] = '';
        $lines[] = '# Apollo.io';
        $lines[] = 'APOLLO_API_KEY=' . $data['apollo_api_key'];
    }
